...
class RQueue {
 ...
 private:
   ...
   ******************************************/
  void insertHelp(Node *root, const Student& input);
  void printHelp(Node *curr) const;
  void clearing(Node *curr);
  Node* mergeHelp(Node* p1, Node* p2);
  Node* order(Node* node);
  void rebuild(Node** array);
  Node* AssignHelp(Node* root); 
  void traverse(Node *root, Node **array);
};
#endif
